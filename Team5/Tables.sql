create database EgyptEducationSystem
--drop database EgyptEducationSystem
...
create table Busses_usedBy_Student(
	buss_number int,
	ssn int,
	primary key(buss_number,ssn),
	foreign key(buss_number) references Busses(buss_number) on delete cascade on update cascade,
	foreign key(ssn) references Students(ssn) on delete no action on update no action
)*/