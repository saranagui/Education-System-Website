...
insert into Courses_havePrerequisite_Courses (course_code1,course_code2)
...
as
select s.school_code as 'elementary_school_code',s.name as 'elementary_school_name',s.main_language as 'elementary_school_main_language',s.vision ,s.mission , s.fees, s.school_type ,s.school_address, s.phone_number , s.email,s.general_info
from Schools s , Elementary_Schools e
where s.school_code=e.school_code
select s.school_code as 'middle_school_code',s.name as 'middle_school_name',s.main_language as 'middle_school_main_language',s.vision ,s.mission , s.fees, s.school_type ,s.school_address, s.phone_number , s.email,s.general_info
from Schools s , Middle_Schools m
where s.school_code=m.school_code
select s.school_code as 'high_school_code',s.name as 'high_school_name',s.main_language as 'high_school_main_language',s.vision ,s.mission , s.fees, s.school_type ,s.school_address, s.phone_number , s.email,s.general_info
from Schools s , High_Schools h
...
as
begin
select S.*,em.*, T.*,p.review from Schools S, Employees em,Teachers T, Parents_writeReviewsAbout_Schools p
...
go
create proc schoolCodeCheck
@school_code int , @username varchar(30), @passedOrNot int output
as
BEGIN
	if(@school_code=(select E.school_code from Employees E where E.username=@username))
		set @passedOrNot=1
	else
		set @passedOrNot=0
end
...
@ssn int, @teacher_username varchar(30),@teacher_password varchar(30),@teacher_school_code int
as
...
@school_code int
as
select * from Children_appliedForBy_Parents_at_Schools
where school_code=@school_code
...
@student_username varchar(30),@student_password varchar(30),@student_school_code int, @student_ssn int
as
update Students
set username=@student_username
where school_code=@student_school_code and ssn=@student_ssn;
update Students
...
@school_code int ,@new_entry varchar(100)
as
update Schools
...
@school_code int ,@new_entry float
as
update Schools
set fees = @new_entry
where school_code=@school_code
...
@school_code int ,@new_entry varchar(40)
as
update Schools
set email = @new_entry
where school_code=@school_code
...
as 
select C.*
from Courses_taughtBy_Teachers CT inner join Employees E
...
as
if(@due_date is null or @content is null or @course_code is null or @assignment_number is null)
...
BEGIN
    ...
    select top 100 ssn from dbo.Courses_TakenBy_Students S
    ...
    BEGIN
	insert into Assignments (course_code2, assignment_number, posting_date, due_date, content, teacher_that_grades_ssn, teacher_that_posts_ssn, ssn)
	values(@course_code, @assignment_number, @post_date, @due_date, @content, @ssn, @ssn, @studentSSN)    
	FETCH NEXT FROM @MyCursor 
    INTO @studentSSN 
    END; 
...
--drop proc viewSolution
go
create proc viewSolution 
@username varchar(30)
as
Select S.id , A.*, S.username
...
--FATYA MEN SASA FOR THE WEBSITE
--view all assignments i posted categorized by the course
--drop proc viewAllAssignments 
go
create proc viewAllAssignments 
@username varchar(30)
as
Select distinct A.assignment_number, A.course_code2, A.posting_date, A.due_date, A.content
from Assignments A inner join Employees E
on A.teacher_that_posts_ssn=E.ssn
where E.username=@username and  A.posting_date is not null and A.due_date is not null
order by A.course_code2
...
@username varchar(30), @course_code varchar(30), @assignment_number int, @student_username varchar(30), @grade float
as
begin
declare @ssn int
...
--drop proc writeReportsAboutStudents
go
create proc writeReportsAboutStudents
@username varchar(30), @studentUsername varchar(30), @teachers_comment varchar(40)
as
begin
declare @student_ssn int
declare @teacher_ssn int
select @student_ssn=S.ssn,@teacher_ssn=E.ssn
...
on CT.teacher_ssn=E.ssn
where E.username=@username and @studentUsername=S.username
if(@teacher_ssn is null or @studentUsername is null or @teachers_comment is null)
...
where P.ssn=@student_ssn
set @issue_date=CURRENT_TIMESTAMP
insert into Reports (teacher_ssn,ssn,teachers_comment,issue_date,parent_username) values (@teacher_ssn,@student_ssn,@teachers_comment,@issue_date,@parent_username)
insert into Reports_about_Students_issuedBy_Teachers(teacher_ssn,ssn) values (@teacher_ssn,@student_ssn)
end
end
...
--view questions
--drop proc viewQuestions
go
create proc viewQuestions 
@teacher_username varchar(30), @course_code varchar(30)
as
Select  Q.course_code, Q.question_id, Q.question_text, Ch.name, S.username
from Courses_taughtBy_Teachers C inner join Employees E
on C.teacher_ssn=E.ssn inner join Questions Q
on Q.course_code=C.course_code inner join Children Ch
on Ch.ssn=Q.ssn inner join Students S
on S.ssn=Ch.ssn
where C.teacher_ssn=E.ssn and Q.question_text is not null and Q.course_code=@course_code and E.username=@teacher_username
...
@course_code varchar(30),@answerText varchar(100),@username varchar(30), @question_id int, @student_username varchar(30)
as
begin
declare @teacher_ssn int
...
Select  @student_ssn=S.ssn
from Courses_TakenBy_Students CS, Students S, Questions Q
where Q.question_text is not null and Q.course_code=@course_code and CS.course_code=@course_code and CS.ssn=S.ssn and S.username=@student_username
--) S
--where S.course_code=@course_code and S.question_id=@questionID
insert into Answers(question_id, ssn,course_code,answer_text,teacher_ssn)
values(@question_id,@student_ssn,@course_code,@answerText,@teacher_ssn)
end
end
...
from Employees E 
where E.username=@username
select S.grade, Ch.name
from Children Ch, Students S, Courses_TakenBy_Students CS, Courses_taughtBy_Teachers C
where CS.ssn=S.ssn and CS.course_code=C.course_code and C.teacher_ssn=@teacher_ssn and Ch.ssn=S.ssn
group by S.grade,  Ch.name
order by Ch.name
end
...
go
CREATE proc applyForChild @pusername varchar(30), @ssn int, @name varchar(30), @birthday datetime,@gender varchar(30), @school_code int
as
if (not exists (select ssn from Children where ssn=@ssn))
begin
insert into Children(ssn,name,birthday,gender)
values (@ssn, @name, @birthday, @gender)
end
insert into Children_appliedForBy_Parents_at_Schools(school_code,ssn,username)
values (@school_code, @ssn, @pusername)
...
as
begin
select S.name as 'School Name',S.school_code as 'School Code',main_language,	vision,	mission,	fees,	school_type,	school_address,	phone_number,	email,	general_info
from Schools_accept_Students SA inner join Schools S
...
if(@school_code in (select school_code from Schools_accept_Students where ssn=@ssn)
	and @ssn not in (select ssn from Students))
begin
...
as
select R.report_id as ID, R.issue_date As 'Issue Date',(E.first_name + E.middle_name + E.last_name) As 'Teacher Name', R.teachers_comment As 'Teacher Comment'
from Teachers T join Employees E 
on T.ssn=E.ssn
join Reports R
...
--NOT RRIGHT 29/11
go
CREATE proc parentReportReply @report_id int, @parent_username varchar(30), @parent_reply varchar(40)
as
update Reports
set parent_reply=@parent_reply
where Reports.parent_username=@parent_username and Reports.report_id=@report_id
...
on T.ssn=R.teacher_ssn
where R.teacher_ssn=@teacher_ssn
...
@ssn int, @name varchar(30), @birthday datetime, @gender varchar(30), @password varchar(30), @grade int, @schoolcode int
as
begin
if(@ssn is not null)
	begin
...
go
CREATE proc loginPeople @username varchar(30), @password varchar(30), @type varchar(30), @passedOrNot int output
as
begin

if(@type='Teacher')
BEGIN
	if(@username in (select E.username from Employees E inner join Teachers T on E.ssn=T.ssn) and @password=(select E.employee_password from Employees E where E.username=@username))
		set @passedOrNot=1
	else
		set @passedOrNot=0
end
...
BEGIN
	if(exists (select E.username from Parents E where E.parent_password=@password and E.username=@username and E.parent_password is not null and E.username is not null) )
	begin
		set @passedOrNot=1
		end
	else
...
BEGIN
	if(exists (select s.username from Students s where s.student_password=@password and s.username=@username and s.student_password is not null and s.username is not null) )
		set @passedOrNot=1
...
BEGIN
	if(@username in (select E.username from Employees E inner join Administrators A on E.ssn=A.administrator_ssn) and @password=(select E.employee_password from Employees E where E.username=@username))
		set @passedOrNot=1
	else
		set @passedOrNot=0
end
...
--FATYA TANYA MEN SASA
go 
create proc checkReviews
@school_code int
as
select s.name, p.review from Parents_writeReviewsAbout_Schools p, Schools s where p.school_code = @school_code and p.school_code=s.school_code